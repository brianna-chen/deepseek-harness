...
async function harness(): Promise<{ gateway: ComeHereGateway; root: string }> {
  const root = await mkdtemp(join(tmpdir(), 'come-here-'))
  roots.push(root)
  ...
  return { gateway: ctx.get('comeHere') as ComeHereGateway, root }
}

describe('ComeHereGateway', () => {
  ...
  it('rejects secrets and symbolic links before copying a skill bundle', async () => {
    ...
    expect(preview.items[0]).toMatchObject({ action: 'reject', reason: 'possible secret detected' })
  })
  ...
  it('applies conflict choices per skill instead of using one global switch', async () => {
    ...
    const preview = await gateway.preview({
      ...
      uploads: [],
      renameSkillConflicts: false,
      skillConflictResolutions: { [id]: 'rename' },
    })
