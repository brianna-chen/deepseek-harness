...
import ComeHereGateway from '../src/index.ts'
import type { ComeHerePlatform } from '../src/types.ts'
...
describe.each(cases)('ComeHere $platform runtime memory', (source) => {
  it('imports source memory into the instruction renderer and skill registry', async () => {
    const root = await mkdtemp(join(tmpdir(), `come-here-${source.platform}-`))
    roots.push(root)
    ...
    const gatewayContext = new Context(); contexts.push(gatewayContext); await gatewayContext.plugin(ComeHereGateway)
    const gateway = gatewayContext.get('comeHere') as ComeHereGateway
